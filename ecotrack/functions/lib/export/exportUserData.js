...
export const exportUserData = onCall(async (request) => {
    ...
    const exportTimes = userData.exportTimes || [];
    ...
    if (format === 'csv') {
        ...
        const activityLines = activities.map(act => `Activity,${act.category},${act.subtype},${act.quantity},${act.co2eKg},${new Date(act.loggedAt.toMillis ? act.loggedAt.toMillis() : act.loggedAt).toISOString()},"${(act.notes || '').replace(/"/g, '""')}"`).join('\n');
        const goalLines = goals.map(g => `Goal,${g.category},${g.type},${g.targetValue},baseline:${g.baselineValue},${new Date(g.targetDate.toMillis ? g.targetDate.toMillis() : g.targetDate).toISOString()},status:${g.status}`).join('\n');
